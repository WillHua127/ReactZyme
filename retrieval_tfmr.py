...
class PretrainedNetwork(nn.Module):
    def __init__(self, mol_input_dim=1024, seq_input_dim=1280, hidden_dim=128, output_dim=64, n_layers=4, dropout=0.0, max_len=2048):
        ...
        self.transformer = nn.Transformer(d_model=hidden_dim, nhead=8, num_encoder_layers=n_layers, num_decoder_layers=n_layers, dim_feedforward=hidden_dim, batch_first=True)
    ...
    def forward(self, mol_src, seq_src):
        ...
        outputs = self.transformer(embedded, embedded) # (B,2T,H)
...
def parse_arguments():
    ...
    parser.add_argument('--model_path', type=str, default='model/time/esm_unimol_epoch49_tfmr', help='checkpoint')
